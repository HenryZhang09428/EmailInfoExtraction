#!/usr/bin/env python3
"""
One-shot generator: creates the lightweight fill_planner.py orchestrator.
Delete this script after running.
"""
import pathlib

DST = pathlib.Path(__file__).resolve().parent.parent / "fill_planner.py"
...
def plan_fill(
    ...
    try:
        ...
        if profile.is_detected:
            ...
            return build_social_security_fill_plan(
                template_schema, extracted_json, llm, template_filename or "", profile, template_intent
            )
    except ImportError:
    ...
    return _plan_insurance_template_with_llm(
        template_schema, extracted_json, llm, template_filename, template_intent
    )
...
def _plan_insurance_template_with_llm(
    ...
    return InsuranceFillPlanner(
        template_schema, extracted_json, llm, template_filename, template_intent, CONFIG
    ).plan()
